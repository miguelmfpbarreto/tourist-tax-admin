...
export async function ChargeListPage({
    ...
    const dobraTotal = result.data
        .filter(function(row) {
            return row.currency === "DOBRA";
        })
        .reduce(function(total, row) {
            return total + Number(row.amount || 0);
        }, 0);

    const euroTotal = result.data
        .filter(function(row) {
            return row.currency === "EURO";
        })
        .reduce(function(total, row) {
            return total + Number(row.amount || 0);
        }, 0);

    const dollarTotal = result.data
        .filter(function(row) {
            return row.currency === "DOLAR";
        })
        .reduce(function(total, row) {
            return total + Number(row.amount || 0);
        }, 0);
    ...
    return (
        <main className="page">
            ...
            <ChargeFilters basePath={basePath} />
